...
class Runtime
{
	...
	__new_function_thread(span, func, args) {
		...
		args = args.concat([new Map()])
		let thread = new _Thread(this.next_thread_id)
		++ this.next_thread_id
		thread.span = span
		thread.func = func
		thread.args = args
		...
		if (this.cur_thread===null)
			this._loop()
		return __new_obj('int', thread.thread_id)
	}
	...
	_prepare_thread_for_call(modname_or_pyobj, func_or_name, argv) {
		...
		if (modname_or_pyobj!==null) {
			...
			if (func_or_name.constructor===String)
				...
				console.error('func_name is expected a string but got '+func_or_name.constructor.name)
		}
		else {
			if (func_or_name.constructor===String)
				func_name = func_or_name
