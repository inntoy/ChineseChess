...
电脑默认的搜索时间为8秒（一般会严重超过这个时间），如果嫌下棋速度太慢，可以在web/py_lib/auto_chess2.py文件夹里找到auto_move函数，找到`board_explore = BoardExplore(board, 8)`，这里的第二个值“8”就是搜索时间，单位是秒。一般来说，搜索时间越长电脑越“聪明”（至少要搜索五步才不会那么傻，搜索六步才会比较聪明）。搜索时间为8秒（实际为十几秒）时程序的棋力勉强等于中等难度的电脑。
